...
export default function AnalysisDashboard({ interview, report, onDownloadReport, onNewAnalysis }: AnalysisDashboardProps) {
  ...
  // Generate sentiment data from sentiment timeline if available, otherwise use Q&A analysis
  const sentimentData = interview.sentimentAnalysis?.sentimentTimeline && interview.sentimentAnalysis.sentimentTimeline.length > 0
    ? interview.sentimentAnalysis.sentimentTimeline.map((item, index) => ({
    ...
    ? report.qaAnalysis
        ...
        .map((qa, index) => {
          ...
          let candidateScore = (report.candidateEngagement || 50) / 10;
          ...
          const recruiterScore = (report.recruiterSentiment || 50) / 10;
  ...
  const handleDownloadPDFReport = async () => {
    ...
    try {
      // Use _id from MongoDB or id from the interface
  ...
  const handleDownloadCandidateReport = async () => {
    ...
    try {
      // Use _id from MongoDB or id from the interface
      const interviewId = (interview as any)._id || interview.id;
      const response = await fetch(`/api/interviews/${interviewId}/candidate-report`);
  ...
  const handleDownloadRecruiterReport = async () => {
    ...
    try {
      // Use _id from MongoDB or id from the interface
      const interviewId = (interview as any)._id || interview.id;
      const response = await fetch(`/api/interviews/${interviewId}/recruiter-report`);
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-8" data-testid="analysis-dashboard">
      {/* Header */}
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-2">Interview Analysis</h1>
            <p className="text-gray-600 dark:text-gray-400">
              Candidate • Recruiter • {new Date(interview.createdAt || Date.now()).toLocaleDateString()}
            </p>
          </div>
        ...
        {(interview.piiRedacted || interview.encryptedTranscript) && (
          <Alert className="bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800 mb-6">
            ...
            <AlertDescription className="text-sm text-green-800 dark:text-green-200 flex items-center gap-4">
              <span>
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-center">
        ...
        <Button 
          ...
          className="w-full sm:w-auto bg-white dark:bg-gray-900 border-gray-300 dark:border-gray-700"
